...
gls_df$coast <- NA
gls_df$coast[gls_df$index %in% levels(droplevels(sites$index[sites$coast == "wc"]))] <- "wc"
gls_df$coast[gls_df$index %in% levels(droplevels(sites$index[sites$coast == "sc"]))] <- "sc"
gls_df$coast[gls_df$index %in% levels(droplevels(sites$index[sites$coast == "ec"]))] <- "ec"
# Create tibble
...
# correlation plots -------------------------------------------------------
pdf(file = "graph/correlations_new.pdf", width = 9, height = 3)
par(mfrow=c(1, 3))
plot(x1$DT_model, x2$DT_model, pch = ".", col = "red", type = "p",
     xlab = "Precision: 0.001", ylab = "Precision: 0.01", cex = 8)
plot(x1$DT_model, x3$DT_model, pch = ".", col = "red", type = "p",
     xlab = "Precision: 0.001", ylab = "Precision: 0.1", cex = 8)
plot(x1$DT_model, x4$DT_model, pch = ".", col = "red", type = "p",
     xlab = "Precision: 0.001", ylab = "Precision: 0.5", cex = 8)
# par(mfrow=c(1, 1))
dev.off()
...
dat_gro <- gls_gro %>%
  select(site, src, DT, DT_model, se_trend, sd_initial, sd_residual,
         p_trend, length, year_index, index, coast) %>%
  unite(fac, site, src, remove = FALSE)
...
dat %>%
  ggplot(aes(x = DT, y = DT_model, group = as.factor(DT))) +
  geom_jitter(aes(col = length, size = length), show.legend = TRUE, width = 0.015, shape = 19) +
  geom_boxplot(fill = "grey20", alpha = 0.35, outlier.colour = NA, size = 0.3) +
  scale_x_continuous(name = expression(paste("Actual trend (", degree, "C/dec)"))) +
  scale_y_continuous(name = expression(paste("Model trend (", degree, "C/dec)")),
                     breaks = c(-0.2, 0, 0.05, 0.1, 0.15, 0.2, 0.4)) +
...
ggsave("graph/all_plt4_no_interp_natural.pdf", plot = last_plot(), width = 5, height = 7, units = "in")


# plotting p-value vs. SD (initial) by coast ------------------------------
## NB: "graph/all_plt4_no_interp_natural_coast.pdf" is also created in "5.Text.R"
dat %>%
  ggplot(aes(x = sd_initial, y = p_trend)) + bw_update +
  geom_hline(yintercept = 0.05, col = "red") +
  geom_point(aes(shape = coast, colour = coast), size = 3, alpha = 0.8) +
  ...
  theme(axis.title = element_text(size = 12),
        ...
        legend.box = "vertical",
        panel.background = element_rect(fill = "grey80"))
ggsave("graph/all_plt4_no_interp_natural_coast.pdf", plot = last_plot(), width = 5, height = 5, units = "in")
...
dat_gro %>%
  ...
  theme(axis.text.x  = element_text(angle = 90, vjust = 0.5),
        panel.background = element_rect(fill = "grey80"))
ggsave("graph/all_plt7_no_interp_grown.pdf", width = 6, height = 3, units = "in")
...
ggsave("graph/non_NA_perc.pdf", height = 6, width = 10, units = "in")
