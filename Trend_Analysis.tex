...
%
%==============================================================================
\abstract{In South Africa 129 \emph{in situ} temperature time series of 1 to 43 years are used for investigations of the thermal characteristics of coastal seawater. They are comprised of temperature recordings at precisions ranging from \SIrange{0.1}{0.001}{\degreeCelsius} and collected with handheld thermometers or underwater temperature recorders (UTRs). Using the naturally occurring range of seasonal signals, temperature trends, and variability for 84 of these time series, the length, decadal trend, and data precision variables of each time series were systematically adjusted before being fitted with generalized least squares (GLS) models to study the effect these variables have on trend detection. We determined that low instrument precision has less effect on the ability of a model to detect trends within a time series than does the length, natural variance or the decadal trend itself, with length contributing the most to detection. We found that time series at least 20 years in length may be used tentatively for climate change research (RWS: This isn't really true. 30 years appears to be the safe minimum.), but that time series 30 years in length or longer are preferable. The implication is that long-running thermometer time series in this dataset, and others around the world, are more useful for decadal scale climate change studies than the shorter, more precise UTR time series.}
...
%The large range of values present in our data has led us to consider how to accurately measure the middle, high and low ends of the variables in this dataset. The mean and median values for most variables throughout this publication are different. These two statistics are informative as to the `shape' of the dataset, meaning that when the mean is greater than the median there are large outliers skewing the distribution to the right, and \emph{vice versa}. With large outliers such as a trend of \SI{-7.233}{\degreeCelsius}~dec$^{-1}$ it was also necessary to account for the effect these outliers could have when calculating a range of values. The use of the 5th and 95th percentile values in place of the minimum and maximum values for some analyses was implemented and this is noted where done. For many of the ranges, however, the minimum and maximum values were reported as found because these outliers show what the full range of each variable was. Where appropriate, the values from time series shorter than 10 years in length have been omitted and this is made clear when done as well.
...
%Before analysing any of the time series in this dataset, we determined a statistical \emph{a posteriori} classification of the thermal regions along the coastline into `natural' groupings based on their temperature characteristics. Classifications yielded by ordinations and cluster analyses conveniently matched the three distinct `coasts' \citep{Smit2013} that often form the basis of manual classifications derived from biogeographical insights: the west coast, dominated by the colder Benguela Current; the east coast, influenced by the warmer Agulhas Current; and the south coast, still predominantly warmer Agulhas Current water, but with more spatial and annual variation than along the east coast \citep{Roberts2005}. This classification provided a natural and meaningful way of partitioning the dataset into three smaller groups for further scrutiny.

%The median August and February temperatures for each time series were chosen for the classifications as these generally represent the coldest and warmest months of the year, respectively \citep{Smit2013}. The standard deviation (SD) for these months was used as well. The fifth and final value used for classification was the range from the 5th percentile of the August temperatures to 95th percentile of the February temperatures from each time series so as to allow the range of temperatures within each time series, excluding the very warmest and coolest temperatures, to play a part in their classification. Mean values and their attendant 95\% confidence intervals (CI) were avoided as these had a tendency to be influenced by clumps of very high or very low data points within a time series. A total of 22 time series were removed at this stage of the analysis because they had either less than three years of data, prohibiting an accurate calculation of median temperature and SD, or where collected at depths exceeding 10 m. The five variables selected for each of the remaining 108 time series were combined into one data frame and standardized before a Euclidean distance matrix was calculated for use in the classification.
...
%Before performing any time series analyses, the missing months of data were filled via linear interpolations. After filling any gaps, the large amounts of seasonal variation and noise that exist naturally within the \emph{in situ} time series were removed. To do so a Seasonal Decomposition of Time Series by Loess (STL), using a periodic window, was applied to each time series used in the coastal classification to remove the seasonal variation and residual noise from the trend. STL decomposes a time series into three constituent parts: seasonal signal, trend and remainder. It first uses \emph{loess} smoothing to find the seasonal component of the time series. After removing the seasonal component the time series is iteratively smoothed until the trend is found. The values removed from the trend during this smoothing process become the third component, the remainder (noise) \citep{Cleveland1990}. We extracted only the trend component of each time series as the seasonal signal and remainder components were not of interest to us in this study. It must be noted here that the `trend' determined by STL is not a straight trend line but rather the temperature values of the time series after the seasonal signal and noise have been removed.

%A simple linear model was then fitted to the trend detected by STL for each time series to calculate the average monthly increase throughout the length of the time series. This also allowed the coefficient of determination (\emph{R}\textsuperscript{2}) to be calculated, quantifying the overall linearity of the decadal trend ($\Delta T$ in \si{\degreeCelsius}~dec$^{-1}$) detected. It is important to note that because linear interpolation was used to fill the gaps in the time series this may have artificially affected the linearity of the time series and unduly increase or decrease the \emph{R}\textsuperscript{2} value, thereby affecting the $\Delta T$ and \emph{p}, as the linear model used to discern these values would be more effective with a more linear signal. A simple linear regression between the \emph{R}\textsuperscript{2} and proportion of missing values (hereafter referred to as NA\%) for all time series was applied to better understand this potential relationship.

%Before conducting further analyses with the $\Delta T$ values calculated for each time series, it is necessary to acknowledge that \citet{Franzke2012} has shown that a variety of time series analysis methods used on the same time series, but with different assumptions about the autocorrelation structure of the data, may produce trends different enough that some appear significant while others do not. This is an important consideration suggesting that STL may produce significantly different results to other time series analysis methods; however, we feel that given the large number of time series used in this paper, using multiple time series analysis methods would be impractical and unnecessary.

%Also of interest was the range of values for the time series from the new and old UTRs, the thermometers and the coastal groupings. The range of the 95\% CI for these values was found to be large, so the minimum, median, and maximum values of each variable are shown in place of a mean and 95\% CI. The minimum and maximum values were used instead of the 5th and 95th percentiles because the outliers present in these groupings were not falsely calculated and allow one to see how dramatic the outliers from short time series may be.
...
%With the range of Pearson correlation coefficients (\emph{r}) between temperature change over time known from each time series, it was now possible to apply \emph{a priori} power analyses to determine how many months of data (\emph{n}) would be necessary to have a power of 0.80 or 0.90 for a given instrument type or geographic location. These \emph{n} values were calculated based on the range of \emph{r} values found within each subgroup in the data. However, \citet{Hoenig2001} illustrate the pitfalls of relying on the results of power analyses. Therefore the results of this power analysis must be interpreted against the results from the following analyses.
